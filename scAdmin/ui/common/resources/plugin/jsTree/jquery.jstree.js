        ...
        $(function () {
            if ($.jstree._themes === false) {
                $("script").each(function () {
                    ...
                    /*
                     * Modified by: Felix.liu
                     * Purpose：to fix the bug by compressed. if it was compressed, we could not find the 'script' mark about 'jquery.jstree'.
                     * Date: January 9(th), 2015
                     */
                    else if (this.src.toString().match(/\/require\/require.js$/)) {
                        $.jstree._themes = this.src.toString().replace(/\/require\/require.js$/, "/jsTree/") + 'themes/';
                        return false;
                    }
                });
            }
                    ...
                    if ($.isArray(js)) {
                        ...
                        $.each(js.data, function (i, m) {
                            ...
                            if (m.operate) {
                                //是否是空对象
                                var isOperate = false;
                                for (var name in m.operate) {
                                    isOperate = true
                                    break;
                                }
                                if (isOperate) {
                                    for (var name in m.operate) {
                                        if (m.operate[name] && m.operate[name].isEnable) {
                                            switch (name) {
                                                case 'permissions':
                                                    //权限设置
                                                    if (m.dataObj['key']) {
                                                        for (var i = 0, len = m.dataObj['key'].length; i < len; i++) {
                                                            //多选框
                                                            tmp.append($tree.createControl({ id: m.treeControl, data: m.dataObj['data'], value: m.dataObj['value'][i], text: m.dataObj['key'][i] }, name));
                                                        }
                                                    } else {
                                                        tmp.append($tree.createControl({ id: m.treeControl, data: m.dataObj['data'] }, null));
                                                    }
                                                    break;
                                                default:
                                                    //新增
                                                    tmp.append($tree.createControl({ id: m.treeControl, data: m.dataObj['data'], icon: (m.operate[name].icon || ''), key: (name || '') }, 'btn'));
                                            }
                                        }
                                    }
                                } else {
