...
var rpcClient, pbEncoder, rpcClientWechat, pbEncoderWechat, rpcClientApp, pbEncoderApp, rpcClientAlipay, pbEncoderAlipay, rpcClientDing, pbEncoderDing;
var _accountMap = {};
/**
 * 消息头
...
WeChatAccount.prototype.sendTemplateMessage = function (customerSearchCondition, templateId, url, content) {
    ...
    return new Promise(function (resolve, reject) {
        rpcClientWechat.sendRequest(pbEncoderWechat.encode(JSON.stringify(req_obj)))
            .then(function (res) {
                var result = pbEncoderWechat.decode(res.content);
...
function onWechatMessage(msg) {
    var msgObj = pbEncoderWechat.decode(msg.content);
    if (msgObj && msgObj.evt_message_from_wechat) {
        var account = _accountMap[msgObj.evt_message_from_wechat.account_id];
        if (account) {
...
SMSAccount.prototype.sendSMSMessage = function (customerSearchCondition, text) {
    ...
    return new Promise(function (resolve, reject) {
        rpcClient.sendRequest(pbEncoder.encode(JSON.stringify(req_obj)))
            .then(function (res) {
                var result = pbEncoder.decode(res.content);
...
APPAccount.prototype.sendAppMessage = function (customerSearchCondition, content, subject) {
    ...
    return new Promise(function (resolve, reject) {
        rpcClientApp.sendRequest(pbEncoderApp.encode(JSON.stringify(req_obj)))
            .then(function (res) {
                var result = pbEncoderApp.decode(res.content);
...
exports.init = function (mq, instanceName, programType) {
    ...
    if (mq) {
        ...
        var queue = mq.declareQueue(instanceName + ".wechat.?", onWechatMessage, false, true);
        ...
        //钉钉
        rpcClientDing = sc.RabbitMQ().createRPCClient(mq, 'cf.ding.rpc');
        if (rpcClientDing) {
            pbEncoderDing = sc.createPBEncoder(protocolPath + "cf_ding.proto.js", "cf.ding", "Message");
            if (!!!pbEncoderDing) {
                console.error('create pb encoder ding failed!');
            }
        } else {
            console.error('create rpc client ding failed!');
        }
        mq.declareExchange('cf.ding.notify', 'topic', false, true);
        var queryDing = mq.declareQueue(instanceName + '.ding.?', onDingMessage, false, true);
        queryDing.declareBinding('cf.ding.notify', '#');
    } else {
...
exports.createWeChatAccount = function (accountId) {
    var account = new WeChatAccount(accountId);
...
exports.createSMSAccount = function (accountId) {
    var account = new SMSAccount(accountId);
    _accountMap[accountId] = account;
    return account;
};
/**
...
exports.createEmailAccount = function (accountId) {
    var account = new EmailAccount(accountId);
    _accountMap[accountId] = account;
    return account;
};
/**
...
exports.createAPPAccount = function (accountId) {
    var account = new APPAccount(accountId);
    _accountMap[accountId] = account;
    return account;
};
/**
 ...
 * @returns {WeChatAccount}
 */
exports.createDingAccount = function (accountId) {
    var account = new DingAccount(accountId);
    _accountMap[accountId] = account;
    return account;
};
/**
...
// 创建钉钉账号
function DingAccount(accountId) {
    this._accountId = accountId;
}
// 继承事件驱动类,通知新消息到达
inherits(DingAccount, EventEmitter);
/**
 * 钉钉回调事件
 */
DingAccount.prototype.onMessage = function (msg) {
    // 触发事件
    this.emit('message', msg);
};
/**
 * 监听钉钉接收回调消息
 * exchange_name cf.ding.notify
 * exchange_type topic
 * route_key account_id.USER_ADD_ORG 等等
 */
function onDingMessage(msg) {
    var msgObj = pbEncoderDing.decode(msg.content);
    if (msgObj && msgObj.ntf_msg_from_ding) {
        var account = _accountMap[msgObj.ntf_msg_from_ding.account_id];
        if (account) {
            if (account instanceof DingAccount) {
                // 转给账户类处理
                account.onMessage(msgObj.ntf_msg_from_ding);
            }
        } else {
            console.log('drop ding message:%s', JSON.stringify(msgObj));
        }
    } else {
        console.error("cann't decode pb msg!");
    }
