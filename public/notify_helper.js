...
exports.sendTemplateNotifyWithMobile=function*(key,mobile,data){
    log.info('sendTemplateNotifyWithMobile starting...');
    log.info('input<->key=%s, mobile=%s, data=%s', key, mobile, JSON.stringify(data));
     if (!key || !mobile || typeof(data) != 'object') {
     ...
     try {
        ...
        let result_template_setting = yield getRPoolSync().query(sql);
        ...
        if (result_template_setting && result_template_setting.length > 0) {
                // 被推送人
                let customer_search_condition = makeCustomerSearchCondition(null, null, mobile, null, null);
                // 消息格式
                ...
                if (result_template_setting[0].model == 1) {
                    // 优先级
                    let res_p = yield * prioritySend(channel, info, customer_search_condition, url, data_origin, result_template_setting);
                    return res_p;
                } else {
                    // 并发
                    let res_c = yield * concurrentSend(channel, info, customer_search_condition, url, data_origin, result_template_setting);
                    return res_c;
                }
 ...
 */
exports.sendTemplateNotify = function * (key, cust_id, data,source_id) {
    ...
    try {
        ...
        if (result_template_setting && result_template_setting.length > 0) {
            ...
            let res_data = yield _private_data.rpcCustomerClient.sendRequest(_private_data.pbCustomerEncoder.encode(req_obj));
            ...
            if (res_obj.res_query_customer && res_obj.res_query_customer.list_customer && res_obj.res_query_customer.list_customer.length > 0) {
                ...
                let customer_search_condition = makeCustomerSearchCondition(customer.cf_account_id, customer.openid, customer.mobile, customer.email, customer.app_device_id,cust_id.toString());
                // 消息格式
                let info = JSON.parse(result_template_setting[0].info);
                // 推送渠道
                ...
                let data_origin = sc.deepCopy(data);
                let data_key_array = [];
                let wechat_detail_info=result_template_setting[0].wechat_detail_info;
                data_origin.forEach(d => {
                    if (d.key == 'url') {
                        url = d.value;
                    } else {
                        if (info.wechat&&info.wechat.format&&info.wechat.format[d.key]) {


                            d.color = info.wechat.format[d.key].color;
                            d.key = info.wechat.format[d.key].value;
                            //update by Cindy ,format wechat detail
                            if(wechat_detail_info&&wechat_detail_info!='')
                            {
                                let key = '%' + d.key + '%';
                                wechat_detail_info=wechat_detail_info.replace(key, d.value);
                            }
                        }
                    }
                    data_key_array.push(d.key);
                ...
                // 检查是否有固定的参数
                if (info.wechat.format&&info.wechat.format.common && Object.keys(info.wechat.format.common).length > 0) {
                    for (var k in info.wechat.format.common) {
                        if (data_key_array.indexOf(k) == -1) {
                            ...
                            if(wechat_detail_info&&wechat_detail_info!='') {
                                let key = '%' + k + '%';
                                wechat_detail_info = wechat_detail_info.replace(key, info.wechat.format.common[k].value);
                            }
                ...
                log.info('wechat content=%s', JSON.stringify(data_origin));
                log.info('wechat_detail_info=%s', wechat_detail_info);
                //update by Cindy ,format wechat detail
                if(wechat_detail_info&&wechat_detail_info!='')
                {
                    info.wechat_detail_info=wechat_detail_info
                }
                data.forEach(d => {
                    d.key = '%' + d.key + '%';
                    info.sms.format = info.sms.format.replace(d.key, d.value);
                ...
                if (result_template_setting[0].model == 1) {
                    // 优先级
                    let res_p = yield * prioritySend(channel, info, customer_search_condition, url, data_origin, result_template_setting,source_id);
                    return res_p;
                } else {
                    // 并发
                    let res_c = yield * concurrentSend(channel, info, customer_search_condition, url, data_origin, result_template_setting,source_id);
                    return res_c;
                }
            } else {
 ...
 * 优先级模式发送消息
 */
function * prioritySend(channel, info, customer_search_condition, url, data, result_template_setting,source_id) {
    ...
    for (var c = 0; c < channel.length; c++) {
        if (channel[c] == 1) {
            // 微信
            let res_wechat_p = yield * sendTemplateMessage(info.wechat.id, customer_search_condition, info.wechat.long_template_id, url, data,info.wechat_detail_info,source_id,result_template_setting[0].key);
        ...
        if (channel[c] == 2) {
            // 短信
            let res_sms_p = yield * sendSMSMessage(info.sms.id, customer_search_condition, info.sms.format,source_id,result_template_setting[0].key);
        ...
        if (channel[c] == 3) {
            // 邮件
            let res_email_p = yield * sendEmailMessage(info.email.id, customer_search_condition, result_template_setting[0].name, info.email.format,source_id,result_template_setting[0].key);
        ...
        if (channel[c] == 4) {
            // APP
            let res_app_p = yield * sendAppMessage(info.app.id, customer_search_condition, info.app.format, result_template_setting[0].name,source_id,result_template_setting[0].key);
...
function * createMessageHistory(channel,message_content,customer_search_condition,source_id,template_key)
{
    log.info('createMessageHistory strat...');
    log.info('input<->channel=%s, message_content=%s, customer_search_condition=%s, source_id=%s, template_key=%s', channel, message_content,JSON.stringify(customer_search_condition),source_id,template_key)

    try {
        let curTime = new Date().format("yyyy-MM-dd hh:mm:ss");
        let obj={
            cust_id:customer_search_condition.id,
            source_id:source_id,
            cf_template_key:template_key,
            status_code:1,
            notify_channel:channel,
            notify_content:message_content,
            notify_time:curTime
        };

        let insertHistorySql = db.makeSQLInsert('sc_sys_message_notify_history', obj);
        log.info('insert message history sql:'+insertHistorySql)
        let dbResult = yield getRPoolSync().query(insertHistorySql);
        if (dbResult && 'affectedRows' in dbResult && dbResult.affectedRows > 0) {
            return dbResult;
        } else {
        ...
        log.error("insert message history error, err=%s", e);
        return app.err.systemError.msg(e);
    }
    finally {
        log.info('createMessageHistory finished...');
    }
 ...
 * 并发模式发送消息
 */
function * concurrentSend (channel, info, customer_search_condition, url, data, result_template_setting,source_id) {
    ...
    for (var c = 0; c < channel.length; c++) {
        if (channel[c] == 1) {
            // 微信
            function_list.push(yield * sendTemplateMessage(info.wechat.id, customer_search_condition, info.wechat.long_template_id, url, data,info.wechat_detail_info,source_id,result_template_setting[0].key));
        }
        if (channel[c] == 2) {
            // 短信
            function_list.push(yield * sendSMSMessage(info.sms.id, customer_search_condition, info.sms.format,source_id,result_template_setting[0].key));
        }
        if (channel[c] == 3) {
            // 邮件
            function_list.push(yield * sendEmailMessage(info.email.id, customer_search_condition, result_template_setting[0].name, info.email.format,source_id,result_template_setting[0].key));
        }
        if (channel[c] == 4) {
            // APP
            function_list.push(yield * sendAppMessage(info.app.id, customer_search_condition, info.app.format, result_template_setting[0].name,source_id,result_template_setting[0].key));
        }
    }
 ...
 */
var makeCustomerSearchCondition = (account_id, openid, mobile, email, device_id,id) => {
    return {
 ...
 * @param wechat_detail_info 组装好的微信模板的详细内容
 * @param source_id 来源ID
 * @param template_key 表cf_template_setting的主键key
 * @returns {*} true或者app.err.SCError
 */
function * sendTemplateMessage(accountId, customerSearchCondition, templateId, url, content,wechat_detail_info,source_id,template_key) {
    ...
    try {
        ...
        let res_data = yield _private_data.rpcCfWechatClient.sendRequest(_private_data.pbCfWechatEncoder.encode(JSON.stringify(req_obj)));
        ...
        if (res_obj.res_template_message_send.errcode == 0) {
            //Update by Cindy ,insert message history
            let history_result = yield * createMessageHistory(1,wechat_detail_info,customerSearchCondition,source_id,template_key);
            if (history_result instanceof app.err.SCError) {
                log.error('Insert wechat message history error,error=%s',JSON.stringify(history_result) );
            }
            //end
            return true;
        } else {
 ...
 * @returns {*} true或者app.err.SCError
 */
function * sendSMSMessage(accountId, customerSearchCondition, text,source_id,template_key) {
    ...
    try {
        ...
        let res_data = yield _private_data.rpcCfServerClient.sendRequest(_private_data.pbCfServerEncoder.encode(JSON.stringify(req_obj)));
        ...
        if (res_obj.res_send_text.errcode == 0) {
            //Update by Cindy ,insert message history
            let history_result = yield * createMessageHistory(2,text,customerSearchCondition,source_id,template_key);
            if (history_result instanceof app.err.SCError) {
                log.error('Insert sms message history error,error=%s',JSON.stringify(history_result) );
            }
            //end
            return true;
        } else {
 ...
 * @returns {*} true或者app.err.SCError
 */
function * sendEmailMessage(accountId, customerSearchCondition, subject, text,source_id,template_key) {
    ...
    try {
        ...
        if (res_obj.res_send_text.errcode == 0) {
            //Update by Cindy ,insert message history
            let history_result = yield * createMessageHistory(3,text,customerSearchCondition,source_id,template_key);
            if (history_result instanceof app.err.SCError) {
                log.error('Insert email message history error,error=%s',JSON.stringify(history_result) );
            }
            //end
            return true;
        } else {
 ...
 * @returns {*} true或者app.err.SCError
 */
function * sendAppMessage(accountId, customerSearchCondition, content, subject,source_id,template_key) {
    ...
    try {
        ...
        let res_data = yield _private_data.rpcCfAppClient.sendRequest(_private_data.pbCfAppEncoder.encode(JSON.stringify(req_obj)));
        ...
        if (res_obj.res_push_message.errcode == 0) {
            //Update by Cindy ,insert message history
            log.info('customerSearchConditioncustomerSearchConditioncustomerSearchConditioncustomerSearchCondition:'+JSON.stringify(customerSearchCondition[0]))
            let history_result = yield * createMessageHistory(4,content,customerSearchCondition[0],source_id,template_key);
            if (history_result instanceof app.err.SCError) {
                log.error('Insert app message history error,error=%s',JSON.stringify(history_result) );
            }
            //end
            return true;
        } else {
