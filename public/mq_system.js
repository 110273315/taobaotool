        ...
        if (json)
        {
            ...
            else if (json.ping)
            {
                if (json.ping.instance_name.length==0 || json.ping.instance_name.indexOf(privateData.instanceName)!=-1)
                {
                   ...
                   // if (sc.isMaster)
                    {
                        msg.pong.host_info=getHostInfo();
                    }
                    msg.pong.process_info=getProcessInfo();
                    privateData.mq.publishMessage("amq.direct",json.ping.replay,privateData.encoder.encode(msg));
                }
            }
            else
            {
...
// 发送基础数据改变通知
exports.publishBaseDataChangedNotify=function(table)
{
    //目前只接受表明,改变明细被丢弃
